...
void PlainSocketImpl::create(bool bStream)
{
	...
	//
	if( (socketFD = socket(AF_INET, m_sockType, 0)) == QC_INVALID_SOCKET)
	{
		static const String err(QC_T("unable to create socket: "));
                                       ...
                                       int port, size_t timeoutMS)
{
	...
	if(isConnected())
		throw SocketException(QC_T("socket already connected"));
	...
	if(::connect(m_rpSocketDescriptor->getFD(), reinterpret_cast<struct sockaddr*>(&sa), sizeof(struct sockaddr_in)) < 0)
	{
		...
		// then we can expect the connect() to fail with QC_EINPROGRESS.  In this case
		// we must perform a select() to wait for the command's completion.
		//
		if(!m_bBlocking && errorNum == QC_EINPROGRESS)
		{
			...
			else
			{
				...
				m_rpSocketDescriptor.release();
				static const String err(QC_T("Connection timed out"));
		...
		if(!bConnected)
		{
			...
			if(errorNum == QC_ENETUNREACH || errorNum == QC_EHOSTUNREACH)
			{
	...
	QC_DBG_ASSERT(m_rpSocketDescriptor);
...
//==============================================================================
AutoPtr<InputStream> PlainSocketImpl::getInputStream() const
{
...
// not yet been created or connected (incl passive sockets).
//==============================================================================
AutoPtr<OutputStream> PlainSocketImpl::getOutputStream() const
{
	testSocketIsValid(true /*check connected*/);
...
//
//==============================================================================
AutoPtr<InetAddress> PlainSocketImpl::getLocalAddress() const
{
	//
...
//
//==============================================================================
AutoPtr<InetAddress> PlainSocketImpl::getInetAddress() const
{
...
//
//==============================================================================
AutoPtr<InetAddress> PlainSocketImpl::getRemoteAddress() const
{
...
void PlainSocketImpl::listen(size_t backlog)
{
	...
	{
		static const String err(QC_T("error listening on socket: "));
		String errMsg = err + NetUtils::GetSocketErrorString();
		throw SocketException(errMsg);
	}
}
...
//
//==============================================================================
AutoPtr<SocketDescriptor> PlainSocketImpl::getSocketDescriptor() const
{
...
void PlainSocketImpl::accept(SocketImpl* pSocket)
{
	...
	while(true)
	{
		...
		{
			static const String err(QC_T("Accept timed out"));
			throw SocketTimeoutException(err);
		}

		if( (socketFD = ::accept(m_rpSocketDescriptor->getFD(), pRemoteAddr, &addrLen)) == QC_INVALID_SOCKET)
		{
			int errorNum = NetUtils::GetLastSocketError();
			if(errorNum == QC_EWOULDBLOCK ||
			   errorNum == QC_ECONNABORTED ||
			   errorNum == QC_EPROTO)
			{
		...
		{
			static const String err(QC_T("accept() returned invalid remoteAddr size"));
	...
	// PlainSocketImpl
	//
	AutoPtr<SocketDescriptor> rpSocketFD = new SocketDescriptor(socketFD);
	((PlainSocketImpl*)pSocket)->setSocketDescriptor(rpSocketFD.get());

	QC_DBG_ASSERT(remoteAddr.sin_family == AF_INET);
...
void PlainSocketImpl::testSocketIsValid(bool bTestConnected) const
{
	...
	{
		const static String err = QC_T("Socket not connected");
		throw SocketException(err);
	}
}
